...
def reconcile(alarm,url,snow_user,snow_passwd,auth):
    ...
    >>> result = reconcile({alarm},'https://'devXXXXX.service-now.com/api/now/table/incident','admin','admin',{auth})
    >>> assert action != ''
    """
    update = {}
    ...
    print "REC: this is the ack from snow %s" % s
    print "REC: this is the ack from imc %s" % i
    print "REC: current id is %s" % alarm['id']
    print "REC: this is the snow ticket %s" % check[0].snow_number
    action = "REC: no switcher selected"

    if s == '1' and i == '1': #   snow is new and imc is acknowledged
        action = "REC: snow is new and imc is acknowledged.....switch 1"
        # udate snow to a 2 (in-Precess)
        # Get the snow incident read values
        snow_url = url+'?number='+check[0].snow_number
        print "REC: This is snow_url %s" % snow_url
        ...
        snow_url = url+'/'+sys_id
        print "REC: This is a PUT url %s " % snow_url
        print  update
        # Update service now incident record
        snowObject = ack_snow(update, snow_url, snow_user, snow_passwd)
        print "REC: this is the return_code from snow %s" % snowObject
    ...
    if s == '1' and i == '7': #   snow is new and imc is closed
        action = "REC: snow is new and imc is closed.....switch 2"
        # Get the snow incident read values
        snow_url = url+'?number='+check[0].snow_number
        # snowObject is a dict of the snow incident
        snowObject = get_snow(snow_url, snow_user, snow_passwd)
    ...
    if s == '2' and i == '0': #   snow is In Process and imc is new
        action = "REC: now is In Process and imc is new.....switch 3"
    ...
    if s == '2' and i == '7':  #   snow is in process and imc is closed
        action = "REC: snow is in process and imc is closed.....switch 4"
    ...
    if s == '7' and i == '0': #   snow is closed and imc is realtime new
        action = "REC: snow is closed and imc is realtime new....switch 5"
        # recover IMC
        # update local to a 7 (Closed)
        # udate IMC to userAckType['1'] acknowledged
    ...
    if s == '7' and i == '1': #   snow is closed and imc is acknowledged
        action = "REC: snow is closed and imc is acknowledged.....switch 6"
        # udate IMC to userAckType['1'] acknowledged
        result = recover_alarm(alarm['id'], auth.creds, auth.url)
            # TODO check this return, write to error db
